...
###### Сборщик мусора
Управляемая куча --> та память, которая выделяется, когда создаётся что-то новое.

Сборщик мусора --> механизм очистки памяти от неиспользуемых объектов. (Автоматический менеджер памяти)

Три этапа работы:
	-Маркировка --> сборщик пробегается по объектам, и смотрит,   какие используются, а какие нет. 
	- Перемещение --> Удаляет неиспользованные объекты путём перемещения. (за счёт старого адреса его размещения заменяет на новый адрес )
	- Уплотнение --> за счёт того, что сборщик мусора перемещает объекты в результате чего получается уменьшение памяти.

Когда объект только помещается в память, ему даётся **нулевое** поколение. 
1 поколение --> объекты, которые пережили первую сборку мусора.
2 поколение или долго живущие объекты --> если объекты переживают вторую сборку мусора.

**Чем ниже поколение, тем вероятнее удаление.**
**Чем выше поколение, тем вероятнее оно останется.** 

Large Object Heap (куча больших объектов) --> Это объекты, которые превышают 85 кб.  (Им сразу присваивается 2-е поколение, т.к. такие объекты очень редко уничтожаются)

Дефрагментация -->процесс перераспределения данных для более эффективного хранения и доступа к ним.

Objects Graph -->структура данных, представляющая набор взаимосвязанных объектов, где одни объекты ссылаются на другие.
Если на какие-то объекты нету ссылок, то такие объекты называются недостижимыми, то есть их можно удалить.

Деструктор (Финализатор)--> последний шанс объекта "убрать за собой" перед тем, как сборщик мусора (GC) окончательно сотрёт его из памяти.

Finalization Queue -->специальная внутренняя очередь, куда сборщик мусора (GC) помещает объекты, у которых **есть деструктор (финализатор)**.

###### Делегаты и события

![[Pasted image 20250515084705.png]]
(Ссылка на метод)

Сигнатура --> тип возвращаемого типа данных, а также тип передаваемой переменной.

В 1 делегате могут хранится ссылки на несколько методов. 

+= добавляет ссылку на метод
-= удаляет ссылку на метод

Перед вызовом делегата надо проверить его на null (через if или namedelegate?.Invoke("Привет"))

События(event) --> это механизм, позволяющий объекту уведомлять другие объекты о произошедшем действии (например, клик кнопки, изменение данных).

![[Pasted image 20250515090824.png]]

![[Pasted image 20250515090845.png]]

![[Pasted image 20250515090852.png]]
